...
var mapValues = function(parts, callback, prefix) {
    var result = {}
...
var inclusive = function(mongoProjection) {
    return isInclusive(mongoProjection)
}

var search = function(documents, query, sort, validate) {
...
function parseQuery(query) {
	...
	*/
	...
	// {$and:[{userId: 123}, {animal: {$in: ['beefalo', 'deerclops']}}]}
    var parts = []
...
//var out = parse({$and:[{userId: '123'}, {animal: {$in: ['beefalo', 'deerclops']}}]})
//console.log(out)

//var newQuery = {$and:[{userId: 123}, {animal: {$in: ['beefalo', 'deerclops']}}]}
//var ret = parse(newQuery).matches({userId: 123, animal: 'deerclops'}) // returns true
//console.log(ret)
